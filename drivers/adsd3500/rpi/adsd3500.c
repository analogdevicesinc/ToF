...
static int adsd3500_power_on(struct device *dev)
{
	...
	if (ret < 0) {
		dev_err(adsd3500->dev, "Read of Chip ID register failed. ret=%d\n",ret);
	}
...
static int adsd3500_parse_dt(struct adsd3500 *priv){
	...
	priv->rst_gpio = gpiod_get_optional(dev, "adsd3500-reset", GPIOD_OUT_LOW);
	if (IS_ERR(priv->rst_gpio)) {
		dev_err(dev, "Unable to get \"adsd3500-reset\" gpio\n");
...
static int adsd3500_probe(struct i2c_client *client)
{
	...
	adsd3500->rst_gpio= devm_gpiod_get_optional(dev, "reset-gpios",
						     GPIOD_OUT_HIGH);
	if (IS_ERR(adsd3500->rst_gpio)) {
		dev_err(dev, "Unable to get \"adsd3500-reset-gpios\" gpio\n");
		return PTR_ERR(adsd3500->rst_gpio);
	}
	...
	if (ret < 0) {
		dev_err(dev, "Could not register v4l2 device1\n");
		goto free_entity;
	}
	adsd3500_power_on(dev);
