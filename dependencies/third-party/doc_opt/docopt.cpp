...
DOCOPT_INLINE
std::map<std::string, value>
docopt::docopt_private(std::string const& doc_public,
...
{
	try {
		return docopt_parse(doc_private, argv, help, !version.empty(), options_first);
	} catch (const std::runtime_error& e) {
		return docopt(doc_public, argv, help, version, options_first);
	}
}
