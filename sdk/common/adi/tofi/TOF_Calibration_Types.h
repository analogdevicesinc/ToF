...
struct CAL_LD_SETTINGS_BLOCK_V1 {
    ...
    uint16_t Checksum;
    uint16_t Settings
        []; // Includes pointers for each mode in nmodes at the top followed by the mode data. Length = nWrites.
};
...
struct CAL_ILLUM_PROFILE_BLOCK_V2 {
    ...
    uint8_t
        ProfileData[]; // Placeholder to read the Profiledata (PNG Data) whose
...
struct CAL_P0BLOCK_V4 {
    ...
    uint16_t OffsetCols;
    float
        P0Coeffs[]; // Row Coeffs (nRows * nRowCoeffs) + Pad (if nRows are odd)
...
struct CAL_RELATIVE_ILLUM_BLOCK {
    ...
    float TiltY;
    uint16_t
        Model[]; // Number of elements called out by nSamples field + Padding
