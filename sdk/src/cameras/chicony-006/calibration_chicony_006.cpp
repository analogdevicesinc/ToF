...
aditof::Status CalibrationChicony006::initialize(
    ...
    for (int i = 0; i < PowerUp_data_size; ++i) {
        m_sensor->writeRegisters(&PowerUp_data[i][0], &PowerUp_data[i][1],
                                    1);
    }
    ...
    for (int i = 0; i < Format_size; ++i) {
        m_sensor->writeRegisters(&Format_data[i][0], &Format_data[i][1], 1);
    }
...
aditof::Status CalibrationChicony006::sensorPowerDown() {
    ...
    for (int i = 0; i < SPD_D_size; ++i) {
        m_sensor->writeRegisters(&SPD_D[i][0], &SPD_D[i][1], 1);
    }
...
aditof::Status CalibrationChicony006::TofSetEmissionEnable(uint16_t unEnable) {
    ...
    for (int i = 0; i < TSEE_D_size; ++i) {
        m_sensor->writeRegisters(&TSEE_D[i][0], &TSEE_D[i][1], 1);
        CEEPROM.wait(30);
    }
...
aditof::Status CalibrationChicony006::TofSendCsTable(uint16_t *punSndTbl,
    ...
    for (ulLoop = 0; ulLoop < ulWords; ulLoop++) {
        ...
        m_sensor->writeRegisters(&addr_v, &data_v, 1);
        punSndCurr += 2;
    }
...
aditof::Status CalibrationChicony006::unWriteTofRamReg(uint16_t *punReg,
    ...
    for (i = 0; i < unSetNum; i++) {
        m_sensor->writeRegisters(&punReg[i], &unReg, 1);
    }
...
aditof::Status CalibrationChicony006::unReadTofRamReg(uint16_t *punRegAddr,
    ...
    if (unRegNum > 0) {
        addr_v = punRegAddr[0];
        m_sensor->readRegisters(&addr_v, &unData0, 1);
        for (uni = 1; uni < unRegNum; uni++) {
            addr_v = punRegAddr[uni];
            m_sensor->readRegisters(&addr_v, &unDataX, 1);
            if (unDataX != unData0)
...
aditof::Status CalibrationChicony006::TofChangeRangeMode(uint16_t unMode) {
    ...
    if (unTalFlag != 0x0000) {
        ...
        for (int i = 0; i < 2; ++i) {
            m_sensor->writeRegisters(&TMC_Data[i][0], &TMC_Data[i][1], 1);
        }
    }
    ...
    m_sensor->writeRegisters(&vd_data[0], &vd_data[1], 1);
    ...
    for (int i = 0; i < 3; ++i)
        m_sensor->writeRegisters(&SE_D[i][0], &SE_D[i][1], 1);
    ...
    m_sensor->writeRegisters(&MD_D[0], &MD_D[1], 1);

    //set LD
    uint16_t LD_D[2][2] = {0};
    CEEPROM.SetLD(LD_D, unLdFlag);
    for (int i = 0; i < 2; ++i)
        m_sensor->writeRegisters(&LD_D[i][0], &LD_D[i][1], 1);
    ...
    if (btal) {
        for (int i = 0; i < 2; ++i)
            m_sensor->writeRegisters(&TAL_D[i][0], &TAL_D[i][1], 1);
    }
