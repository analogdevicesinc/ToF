...
aditof::Status Adsd3100Sensor::setControl(const std::string &control,
    ...
    if (control == "modeInfoVersion") {
        ModeInfo::getInstance()->setImagerTypeAndModeVersion(1,
                                                             std::stoi(value));
        return status;
    }
...
aditof::Status Adsd3100Sensor::adsd3500_write_cmd(uint16_t cmd, uint16_t data) {
    LOG(INFO) << "Adsd3500 is not connected to this sensor type!";
    return aditof::Status::UNAVAILABLE;
}
...
aditof::Status Adsd3100Sensor::initTargetDepthCompute(uint8_t *iniFile,
