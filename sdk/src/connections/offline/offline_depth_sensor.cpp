...
aditof::Status OfflineDepthSensor::getFrame(uint16_t *buffer) {
    ...
    if(frame == m_frameTypes.end()){
        LOG(WARNING)
            <<"get frame error "
            << m_frameTypeSelected << " is not found";
...
aditof::Status OfflineDepthSensor::open() {
    ...
    for (int index = 0; index < frameTypesResources.size(); index++){
        std::ifstream ifs;
        ifs.open(m_path + '/' + frameTypesResources[index], std::ios::in|std::ios::binary);
        std::string content((std::istreambuf_iterator<char>(ifs)),
                            (std::istreambuf_iterator<char>()));
        buffer = new uint16_t[content.size()];
        for (int i = 0; i < content.size(); i += 2) {
            buffer[i / 2] = *(uint16_t *)&content[i];
        }
        m_frameTypes.insert({frameTypesResources[index], std::make_pair(buffer, content.size())});
        ifs.close();
    }
    return aditof::Status::OK; 
}
