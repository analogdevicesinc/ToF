...
namespace docopt {
	...
	/// Parse user options from the given private doc string (allowing for hidden undocumented options), and exit appropriately
	/// @param doc_public   The usage string use for display only, displayed for help or in exit messages
	/// @param doc_private  The usage string actually used for parsing, may contain hideen options not included in public doc string
	///
	/// Calls 'docopt_parse' and will terminate the program if any of the exceptions above occur:
	///  * DocoptLanguageError - print error and terminate (with exit code -1)
	///  * DocoptExitHelp - print PUBLIC usage string and terminate (with exit code 0)
	///  * DocoptExitVersion - print version and terminate (with exit code 0)
	///  * DocoptArgumentError - print error and PUBLIC usage string and terminate (with exit code -1)
	std::map<std::string, value> DOCOPT_API docopt_private(std::string const& doc_public,
						std::string const& doc_private,
					    std::vector<std::string> const& argv,
					    bool help = true,
					    std::string const& version = {},
					    bool options_first = false) noexcept;							
