...
#
# input_bin_fn contains 1SP Code + 1SP Text only
#
def bin2newton_cmd(input_bin_fn, output_txt_fn, prod1sp_fuse_files=[]):
    ...
    if prod1sp_fuse_files: 
        ...
        body = override_prod1sp_fuse_data(body, prod1sp_fuse_files)
        out_bin_fn = input_bin_fn.replace('.bin', '-fusedata.bin')
        print('Writing 1SP bin with fuse data')
        with open(out_bin_fn, 'wb') as out_file:
            out_file.write(body)

    # TODO: Override header section based on SP_Bin_Format_Header_t structure

    with open(output_txt_fn, 'w') as out_file:
        out_file.write(get_cmd_1sp_header(len(body)))
