...
std::string getADIFileName(void *hwndOwner, const char *customFilter, char *filename, int &FilterIndex) {
    ...
    while (customFilter[filterLength] != '\0' || customFilter[filterLength + 1] != '\0') {
        filterLength++;
    }
    ...
    std::string command = std::string(zenityP) +
    ...
    for (size_t i = 0; i < filters.size(); i += 2) {
        if (i + 1 < filters.size()) {
            ...
            command += " --file-filter=\"" + filters[i] + " | " + filters[i + 1] + "\"";
        }
    }
    ...
    if (extensionPos == std::string::npos || extensionPos == result.size() - 1) {
        ...
        if (!filters.empty() && filters.size() >= 2) {
            ...
            if (wildcardPos != std::string::npos) {
                defaultExtension = defaultExtension.substr(wildcardPos + 1); // Extract the extension (e.g., ".bin")
    ...
    for (size_t ix = 0; ix < filters.size(); ix += 2) {
        std::string filterExt = filters[ix + 1].substr(filters[ix + 1].find('.') + 1);
