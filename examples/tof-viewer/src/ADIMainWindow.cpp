...
ADIMainWindow::~ADIMainWindow()
{
	...
	//Recording flags
	if (view != nullptr && (view->m_ctrl->m_recorder->isFSFRecording || !view->m_ctrl->m_recorder->m_finishRecording))
	{
...
bool ADIMainWindow::startImGUI(const ADIViewerArgs& args)
{
	...
	string _title = "Analog Devices, Inc. Time of Flight Main Window";//Default name
	if (!files.empty())
	{
		//Make a match
	...
	window = glfwCreateWindow(1280, 720, _title.c_str(),
		NULL, NULL);
	...
	{//Look for icons
		for (size_t count = 0; count < files.size(); count++)
		{
			if (files[count] == _iconName)
			{
				...
				icons[0].pixels = stbi_load(iconPath.c_str(), &icons[0].width, &icons[0].height, NULL, 4);
	...
	for (size_t count = 0; count < files.size(); count++)
	{
		...
		{
			logos[0].pixels = stbi_load(iconPath.c_str(), &logos[0].width, &logos[0].height, NULL, 4);
			...
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, logos[0].width, logos[0].height, 0, GL_RGBA, GL_UNSIGNED_BYTE, logos[0].pixels);
...
void ADIMainWindow::render()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (_fsfShowPbOpWin)
		{
			ShowFSFOptionsGUI(mainWindowWidth, mainWindowHeight, _fsfShowPbOpWin, _startFSFPb, view->m_ctrl->m_recorder->_streamEnable);			
		}
...
void ADIMainWindow::RefreshDevices()
{
	...
	const std::vector<std::pair<size_t, std::vector<std::pair<uint16_t, uint16_t>>>> installedDevices = getInstalledDeviceCount();
	...
	{		
		...
		case ADI_CCD_VID:			
			...
			break;
		case ADI_FPGA_VID:
			m_connectedDevices.emplace_back(i, "ToF Camera " + std::to_string(i));
			break;
		case FAKE_VID://Temp case
			m_connectedDevices.emplace_back(i, "Fake Device " + std::to_string(i));
			break;
		default:
			break;
		}
...
std::vector<std::pair<size_t, std::vector<std::pair<uint16_t, uint16_t>>>> ADIMainWindow::getInstalledDeviceCount()
{
	size_t device_count = 0;
	std::vector<std::pair<uint16_t, uint16_t>> devicePIDVID;
	std::vector<std::pair<size_t, std::vector<std::pair<uint16_t, uint16_t>>>> _deviceCount_PIDVID;
...
void ADIMainWindow::showOpenDeviceWindow()
{
	...
	ImGui::Begin("Device",NULL, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar);
	
	if (ImGui::TreeNode("Open Device"))
	{
		ImGuiExtensions::ADIComboBox("Device",
		...
		const bool openAvailable = !m_connectedDevices.empty();
		{
			...
			if (ImGuiExtensions::ADIButton("Open Device", /*openAvailable*/_isOpenDevice && m_configFiles.size() > 0) && 0 <= m_selectedDevice)
			{
				if (isPlayRecorded)
				{
					stopPlayback();
		...
		if (!m_connectedDevices.empty())
		{
			...
			ImGuiExtensions::ADIComboBox("Config Files",
				"No Config Files",
		...
		if (isADICCD)
		{
			...
			ImGui::Separator();
			ImGui::NewLine();
			ImGui::SetNextTreeNodeOpen(cameraOptionsTreeEnabled, ImGuiCond_FirstUseEver);
		...
		if (isADIToF && cameraWorkerDone)
		{
			...
			ImGui::Separator();
			ImGui::NewLine();
			ImGui::SetNextTreeNodeOpen(cameraOptionsTreeEnabled, /*ImGuiCond_FirstUseEver*/ImGuiCond_Always);
	...
	ImGui::Begin("Company Logo", NULL, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar);
	...
	if (logos[0].pixels != nullptr)
	{
		...
		ImGui::Image((void*)(intptr_t)logo_texture, ImVec2(logos[0].width* scale, logos[0].height* scale));
	}
...
void ADIMainWindow::ShowPlaybackTree()
{
	...
	if (ImGui::TreeNode("Playback Options"))
	{
		...
		ImGuiExtensions::ButtonColorChanger colorChangerPlay(customColorOpenRec, !isPlayRecorded && !isPlaying);

		if (ImGuiExtensions::ADIButton("Open Recording", !isPlayRecorded && !isPlaying && !_fsfShowPbOpWin))
		{
			customFilter = "FSF Files (*.fsf)\0*.fsf*\0 Raw Files (*.raw)\0*.raw*\0 All Files (*.*)\0*.*\0";
		...
		if (_getFSFInfo)
		{
			...
			view->m_ctrl->m_recorder->_streamEnable = checkAvailability;//init this variable again.
		...
		ImGui::SameLine();
		ImGuiExtensions::ButtonColorChanger colorChangerPausePB(recordPlaybackColor, isPlayRecorded && isPlayRecordPaused);
		...
		ImGuiExtensions::ButtonColorChanger colorChangerStopPB(customColorStop, isPlayRecorded);
		...
		if (isPlayRecorded)
		{
			...
			ImGui::Text("View Options:");
			ImGuiExtensions::ADIRadioButton("Active Brightness and Depth", &viewSelection, 0, displayIR || displyaDepth);
			ImGui::NewLine();
		...
		if (0)
		{
			...
			{
				customFilter = "Raw Files (*.bin)\0*.bin*\0 All Files (*.*)\0*.*\0";
				string path = openADIFileName().c_str();
				if (!path.empty())
				{
					if (view == NULL)
					...
					if (path.find(".bin") != std::string::npos)//If bin File was found
					{
						...
						int width = 1024;
						int height = 1024;
						view->m_ctrl->startPointCloudBinToFSFConversion(path, frames, width, height);
					}
				}
			}
		}
...
void ADIMainWindow::ShowRecordTree()
{
	...
	{
		ImGuiExtensions::ButtonColorChanger colorChangerStartRec(customColorPlay, isPlaying);
		...
		if (ImGuiExtensions::ADIButton("Start Recording", !isRecording && !isPlayRecorded))
		{
			...
			if (!saveFile.empty())
			{
				...
				case 2://Raw file
					...
					view->m_ctrl->startRecording(saveFile, view->frameHeight, view->frameWidth, recordingSeconds);
					isRecording = true;
					break;
		...
		if (_startFSFRec)
		{
			...
			view->m_ctrl->startRecording(fsfPath, view->frameHeight, view->frameWidth, recordingSeconds);			
			isRecording = true;
			_startFSFRec = false;
		}
...
void ADIMainWindow::PlayRecorded()
{
	...
	if (_usingFSF)
	{
		...
		displayPointCloud = view->m_ctrl->m_recorder->_streamEnable.x && view->m_ctrl->m_recorder->_streamEnable.y && view->m_ctrl->m_recorder->_streamEnable.depth;
		...
		{//We're maybe displaying only Point Cloud
			displayIR = false;
			displyaDepth = true;
...
void ADIMainWindow::displayActiveBrightnessWindow(ImGuiWindowFlags overlayFlags)
{
...
void ADIMainWindow::displayDepthWindow(ImGuiWindowFlags overlayFlags)
{
	...
	if (ImGui::Begin("Depth Window", nullptr, overlayFlags))
	{
		...
		GetHoveredImagePix(hoveredImagePixel, ImGui::GetCursorScreenPos(), ImGui::GetIO().MousePos, displayDepthDimensions);

		RenderInfoPane(hoveredImagePixel, view->depth_video_data, view->frameWidth, ImGui::IsWindowHovered(), ADI_Image_Format_t::ADI_IMAGE_FORMAT_DEPTH16, "mm");
	}
...
void ADIMainWindow::displayPointCloudWindow(ImGuiWindowFlags overlayFlags)
{
	...
	{		
		...
		if (view->frameHeight == 512)
		{
			ImVec2 newPCDimensionQmp = { static_cast<float>(displayPointCloudDimensions.x * 1.37), static_cast<float>(displayPointCloudDimensions.y * 1.3) };
			ImGui::SetWindowSize(newPCDimensionQmp);
		}
		else
		{
			ImVec2 newPCDimension = { static_cast<float>(displayPointCloudDimensions.x * 1.12), static_cast<float>(displayPointCloudDimensions.y * 1.15) };
...
void ADIMainWindow::preparePointCloudVertices(unsigned int& vbo, unsigned int& vao)
{
	...
	glBufferData(GL_ARRAY_BUFFER, view->vertexArraySize, view->normalized_vertices, GL_DYNAMIC_DRAW);//GL_STATIC_DRAW 
...
void ADIMainWindow::GetHoveredImagePix(ImVec2& hoveredImagePixel, ImVec2 imageStartPos, ImVec2 mousePos, ImVec2 displayDimensions)
{
...
void ADIMainWindow::RenderInfoPane(ImVec2 hoveredImagePixel, uint16_t* currentImage, int imageWidth, bool isHovered, ADI_Image_Format_t format, std::string units)
{
...
void ADIMainWindow::CaptureDepthVideo()
{
	if (view->depth_video_data_8bit != nullptr)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, view->frameWidth, view->frameHeight, 0, GL_BGR, GL_UNSIGNED_BYTE, view->depth_video_data_8bit);
...
void ADIMainWindow::CaptureIRVideo()
{
	if (view->ir_video_data_8bit != nullptr)
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, view->frameWidth, view->frameHeight, 0, GL_BGR, GL_UNSIGNED_BYTE, view->ir_video_data_8bit);
		glGenerateMipmap(GL_TEXTURE_2D);
...
void ADIMainWindow::CapturePointCloudVideo()
{
	...
	mat4x4_perspective(m_projection, radians(fov), (float)view->frameWidth / (float)view->frameHeight, 0.1f, 100.0f);
...
void ADIMainWindow::processInputs(GLFWwindow* window)
{
	...
	if (mouseDown)
	{
		...
		//
		if (mouseDownPos.x >= 0.f && mouseDownPos.x <= displayPointCloudDimensions.x && mouseDownPos.y >= 0.f &&
...
void ADIMainWindow::GetArcballRotation(quat rotation, const vec2 displayDimensions, const vec2 startPos, const vec2 endPos)
{
	vec3 startVector;
