...
aditof::FSF *ADIBinToFSF::ConvertPointCloudToFSF(const std::string &fileName,
    ...
    if (readFile.is_open()) {
        readFile.seekg(0);
        ; //Start from the beginning. It does not have any header (for now)
        readFile.read(reinterpret_cast<char *>(frameDataLocationXYZ), fileSize);
        readFile.close();
    }
    fsfWrite = new aditof::FSF_Common{aditof::FsfMode::WRITE,
                                      (const unsigned int)frames};
    if (fsfWrite->CreateFsfFile(fileName.c_str()) !=
    ...
    fileHeader.FrameOffsetInfoLoc =
        100; //167773443;//16774423;//167773651;//Verify this
    ...
    if (fsfWrite->SetOptionalFileHeader(optFileHeader) !=
        aditof::FsfStatus::SUCCESS) {
    ...
    if (_streamEnable.depth) {
        ...
        stream.streamData.assign(reinterpret_cast<const char *>(zData),
                                 reinterpret_cast<const char *>(zData) + zCnt);
        if (fsfWrite->SetStream(
                /*frameCtr*/ 0,
                /*streamTable.find((uint32_t)aditof::StreamType::STREAM_TYPE_DEPTH)->second*/
                0, stream) != aditof::FsfStatus::SUCCESS) {
            LOG(ERROR) << "Could not set stream.";
        }
        ...
        if (_streamEnable.depth && _streamEnable.x && _streamEnable.y) {
            ...
            stream.streamData.assign(reinterpret_cast<const char *>(xData),
                                     reinterpret_cast<const char *>(xData) +
                                         xCnt);
            int xIndex =
                1; //streamTable.find((uint32_t)aditof::StreamType::STREAM_TYPE_X)->second;
            if (fsfWrite->SetStream(/*frameCtr*/ 0, xIndex, stream) !=
                aditof::FsfStatus::SUCCESS) {
                LOG(ERROR) << "Could not set stream.";
                delete xData;
                delete yData;
            ...
            stream.streamData.assign(reinterpret_cast<const char *>(yData),
                                     reinterpret_cast<const char *>(yData) +
                                         yCnt);
            int yIndex =
                2; //streamTable.find((uint32_t)aditof::StreamType::STREAM_TYPE_Y)->second;
            if (fsfWrite->SetStream(/*frameCtr*/ 0, yIndex, stream) !=
                aditof::FsfStatus::SUCCESS) {
                LOG(ERROR) << "Could not set stream.";
                delete xData;
                delete yData;
