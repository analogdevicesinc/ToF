...
ADIToFRecorder::ADIToFRecorder()
    : m_frameDetails{"", {}, "", 0, 0, 0, false}, m_recordTreadStop(true),
      ...
      m_playBackEofReached(false), m_numberOfFrames(0), fsfFileName(""),
      fsfFileNameRaw("") {}
...
void ADIToFRecorder::startRecording(const std::string &fileName,
                                    ...
                                    unsigned int fps) {
    if (fileName.find(".fsf") !=
        std::string::npos) { //This is an FSF file, then go to FSF Recording
        ...
        if (pos != std::string::npos) {
            ...
            if (GetFullPathName(rawFileDirectory.c_str(), max_path,
                                &dir_path[0], NULL) == 0) {
...
void ADIToFRecorder::startRecordingFSF(const std::string &fileName, int &fps,
                                       unsigned int height,
                                       unsigned int width) {
    ...
    pFsfWrite =
        new aditof::FSF_Common{aditof::FsfMode::WRITE, (const unsigned int)fps};
    pFsfWriteRaw =
        new aditof::FSF_Common{aditof::FsfMode::WRITE, (const unsigned int)fps};
    if (pFsfWrite->CreateFsfFile(fsfFileName.c_str()) !=
        aditof::FsfStatus::SUCCESS) {
        LOG(ERROR) << "Could not create FSF file";
        return;
    }

    createRawFsfFile(fsfFileNameRaw, height, width,
                     m_frameDetails.totalCaptures, (uint32_t)fps);
    ...
    if (_streamEnable.radial) //Depth
        _numStreams++;
    fileHeader.nStreams =
        _numStreams; //This variable can't be put after pFsfWrite->SetStreamInfo function. It will create an exception
    if (pFsfWrite->SetFileHeader(fileHeader) != aditof::FsfStatus::SUCCESS) {
        LOG(ERROR)
            << "Could not create FSF file, error while creating file header.";
        return;
    }
    ...
    if (_streamEnable.active_br) {
        ...
        streamInfo.StreamType =
            (uint32_t)aditof::StreamType::STREAM_TYPE_ACTIVE_BR;
        streamInfo.ChannelFormat =
            (uint32_t)aditof::ChannelFormat::FSF_CHANNEL_UINT16;
        ...
        if (pFsfWrite->SetStreamInfo(_streamCounter, streamInfo) !=
            aditof::FsfStatus::SUCCESS) {
            LOG(ERROR) << "Could not create FSF file, error while creating "
                          "stream info for ACTIVE_BR.";
            return;
        }
        streamTable[(uint32_t)aditof::StreamType::STREAM_TYPE_ACTIVE_BR] =
            _streamCounter;
        _streamCounter++;
    }
    ...
    if (_streamEnable.radial) //Only Depth
    {
        ...
        streamInfo.SystemID = 0;
        streamInfo.StreamType =
            (uint32_t)aditof::StreamType::STREAM_TYPE_RADIAL;
        streamInfo.ChannelFormat =
            (uint32_t)aditof::ChannelFormat::FSF_CHANNEL_SIGNED_INT16;
        streamInfo.BytesPerPixel = 2;
        streamInfo.nRowsPerStream = height;
        streamInfo.nColsPerStream = width;
        streamInfo.OptionalStreamHdrSize = 3;
        ...
        if (pFsfWrite->SetStreamInfo(_streamCounter, streamInfo) !=
            aditof::FsfStatus::SUCCESS) {
            LOG(ERROR) << "Could not create FSF file, error while creating "
                          "stream info for TYPE_RADIAL.";
            return;
        }
        streamTable[(uint32_t)aditof::StreamType::STREAM_TYPE_RADIAL] =
            _streamCounter;
        _streamCounter++;
    }
    ...
    if (_streamEnable.x) {
        ...
        if (pFsfWrite->SetStreamInfo(_streamCounter, streamInfo) !=
            aditof::FsfStatus::SUCCESS) {
            LOG(ERROR) << "Could not create FSF file, error while creating "
                          "stream info for TYPE_X.";
            return;
        }
        streamTable[(uint32_t)aditof::StreamType::STREAM_TYPE_X] =
            _streamCounter;
        _streamCounter++;
    }
    if (_streamEnable.y) {
        ...
        if (pFsfWrite->SetStreamInfo(_streamCounter, streamInfo) !=
            aditof::FsfStatus::SUCCESS) {
            LOG(ERROR) << "Could not create FSF file, error while creating "
                          "stream info for TYPE_Y.";
            return;
        }
        streamTable[(uint32_t)aditof::StreamType::STREAM_TYPE_Y] =
            _streamCounter;
        _streamCounter++;
    }
    ...
    if (_streamEnable.depth) {
        ...
        if (pFsfWrite->SetStreamInfo(_streamCounter, streamInfo) !=
            aditof::FsfStatus::SUCCESS) {
            LOG(ERROR) << "Could not create FSF file, error while creating "
                          "stream info for TYPE_DEPTH.";
            return;
        }
        streamTable[(uint32_t)aditof::StreamType::STREAM_TYPE_DEPTH] =
            _streamCounter;
        _streamCounter++;
    }
    ...
    if (pFsfWrite->SetOptionalFileHeader(optFileHeader) !=
        aditof::FsfStatus::SUCCESS) {
        LOG(ERROR) << "Could not create FSF file, error on setting optional "
                      "header file.";
        return;
    }
    ...
    if (pFsfWrite->SetFileComment(fileComment) != aditof::FsfStatus::SUCCESS) {
        LOG(ERROR)
            << "Could not create FSF file, error on setting file comment.";
        return;
    }
...
void ADIToFRecorder::createRawFsfFile(std::string &fileName,
                                      unsigned int height, unsigned int width,
                                      uint8_t totalCaptures, uint32_t nFrames) {
    if (pFsfWriteRaw->CreateFsfFile(fileName.c_str()) !=
        aditof::FsfStatus::SUCCESS) {
    ...
    fileHeaderRaw.nStreams =
        totalCaptures; //This variable can't be put after pFsfWrite->SetStreamInfo function. It will create an exception
    if (pFsfWriteRaw->SetFileHeader(fileHeaderRaw) !=
        aditof::FsfStatus::SUCCESS) {
        LOG(ERROR)
            << "Could not create BIN file, error while creating file header.";
        return;
    }
    ...
    streamInfoRaw.StreamType =
        (uint32_t)aditof::StreamType::STREAM_TYPE_RAW_NORM;
    streamInfoRaw.ChannelFormat = static_cast<uint32_t>(
        aditof::ChannelFormat::FSF_CHANNEL_SIGNED_INT16); //Verify this concept
    ...
    for (size_t streamCtr = 0; streamCtr < totalCaptures; streamCtr++) {
        if (pFsfWriteRaw->SetStreamInfo(streamCtr, streamInfoRaw) !=
            aditof::FsfStatus::SUCCESS) {
            LOG(ERROR) << "Could not create FSF file, error while creating "
                          "stream info for STREAM_TYPE_RAW.";
            return;
        }
    }
    ...
    if (pFsfWriteRaw->SetOptionalFileHeader(optFileHeaderRaw) !=
        aditof::FsfStatus::SUCCESS) {
        LOG(ERROR) << "Could not create FSF file, error on setting optional "
                      "header file.";
        return;
    ...
    //File Comment
    fileComment = "";
    if (pFsfWriteRaw->SetFileComment(fileComment) !=
        aditof::FsfStatus::SUCCESS) {
        LOG(ERROR)
            << "Could not create FSF file, error on setting file comment.";
        return;
...
int ADIToFRecorder::startPlayback(const std::string &fileName, int &fps) {
    ...
    } else if (fileName.find(".fsf") !=
               std::string::npos) //If fsf File was found
    {
        pFsfRead = new aditof::FSF_Common{aditof::FsfMode::READ,
...
bool ADIToFRecorder::startPointCloudBinToFSFConversion(
...
void ADIToFRecorder::startPlaybackFSF() {
    ...
    if (nullptr != pFsfRead) {
        ...
        if (pFsfRead->GetFileHeader(fileHeader) != aditof::FsfStatus::SUCCESS) {
            LOG(ERROR)
                << "Could not load FSF file, error on getting file header.";
            return;
        }

        for (int idx = 0; idx < fileHeader.nStreams; idx++) {
            if (pFsfRead->GetStreamInfo(idx, streamInfo) !=
                aditof::FsfStatus::SUCCESS) {
                LOG(ERROR) << "Could not load FSF file, error on getting "
                              "stream info for %d index.",
                    idx;
                return;
            }
        ...
        if (pFsfRead->GetOptionalFileHeader(optFileHeader) !=
            aditof::FsfStatus::SUCCESS) {
            LOG(ERROR) << "Could not load FSF file, error on getting optional "
                          "file header.";
            return;
        }
        //Get File Comment
        if (pFsfRead->GetFileComment(fileComment) !=
            aditof::FsfStatus::SUCCESS) {
            LOG(ERROR)
                << "Could not load FSF file, error on getting file comment.";
            return;
        }
...
void ADIToFRecorder::recordFSFThread() {
    while (!m_recordTreadStop) {
        ...
        size_t size =
            frameDetails.height * frameDetails.width * sizeof(uint16_t);
        ...
        if (_streamEnable.active_br) {
            ...
            stream.streamData.assign(reinterpret_cast<const char *>(irData),
                                     reinterpret_cast<const char *>(irData) +
                                         size);
            if (pFsfWrite->SetStream(
                    frameCtr,
                    streamTable
                        .find(
                            (uint32_t)aditof::StreamType::STREAM_TYPE_ACTIVE_BR)
                        ->second,
                    stream) != aditof::FsfStatus::SUCCESS) {
        ...
        if (_streamEnable.radial) {
            ...
            stream.streamData.assign(reinterpret_cast<const char *>(depthData),
                                     reinterpret_cast<const char *>(depthData) +
                                         size);
            if (pFsfWrite->SetStream(
                    frameCtr,
                    streamTable
                        .find((uint32_t)aditof::StreamType::STREAM_TYPE_RADIAL)
                        ->second,
                    stream) != aditof::FsfStatus::SUCCESS) {
                LOG(ERROR) << "Could not set stream.";
                recordFSFThreadFail();
            ...
            if (_streamEnable.depth && _streamEnable.x && _streamEnable.y) {
                ...
                uint16_t *
                    xyzData; //The whole Point Cloud buffer, however, we need to break it out into x and y for backwards compatibility
                uint16_t *xData =
                    new uint16_t[frameDetails.height * frameDetails.width *
                                 sizeof(uint16_t)];
                uint16_t *yData =
                    new uint16_t[frameDetails.height * frameDetails.width *
                                 sizeof(uint16_t)];
                uint16_t *zData =
                    new uint16_t[frameDetails.height * frameDetails.width *
                                 sizeof(uint16_t)];
                size_t pcsize = frameDetails.height * frameDetails.width *
                                sizeof(aditof::Point3I);
                size_t xBuffCnt = 0;
                size_t yBuffCnt = 0;
                size_t zBuffCnt = 0;
                ...
                //X component
                stream.streamData.assign(reinterpret_cast<const char *>(xData),
                                         reinterpret_cast<const char *>(xData) +
                                             xBuffCnt);
                int xIndex =
                    streamTable
                        .find((uint32_t)aditof::StreamType::STREAM_TYPE_X)
                        ->second;
                if (pFsfWrite->SetStream(frameCtr, xIndex, stream) !=
                    aditof::FsfStatus::SUCCESS) {
                    LOG(ERROR) << "Could not set stream.";
                    recordFSFThreadFail();
                    delete[] xData;
                ...
                stream.streamData.assign(reinterpret_cast<const char *>(yData),
                                         reinterpret_cast<const char *>(yData) +
                                             yBuffCnt);
                int yIndex =
                    streamTable
                        .find((uint32_t)aditof::StreamType::STREAM_TYPE_Y)
                        ->second;
                if (pFsfWrite->SetStream(frameCtr, yIndex, stream) !=
                    aditof::FsfStatus::SUCCESS) {
                    LOG(ERROR) << "Could not set stream.";
                    recordFSFThreadFail();
                    delete[] xData;
                ...
                stream.streamData.assign(reinterpret_cast<const char *>(zData),
                                         reinterpret_cast<const char *>(zData) +
                                             zBuffCnt);
                int zIndex =
                    streamTable
                        .find((uint32_t)aditof::StreamType::STREAM_TYPE_DEPTH)
                        ->second;
                if (pFsfWrite->SetStream(frameCtr, zIndex, stream) !=
                    aditof::FsfStatus::SUCCESS) {
                    LOG(ERROR) << "Could not set stream.";
                    recordFSFThreadFail();
                    delete[] xData;
        ...
        for (size_t streamCnt = 0; streamCnt < fileHeaderRaw.nStreams;
             streamCnt++) {
            streamRaw.streamHeader.TimeStamp = frameCtr;
            streamRaw.streamHeader.CompressedStreamSize = size;
            streamRaw.optionalStreamHeader.assign(
                reinterpret_cast<const char *>(pHeader) +
                    (EMBED_HDR_LENGTH * streamCnt),
                EMBED_HDR_LENGTH);
            streamRaw.streamComment = "RAW";
            streamRaw.streamData.assign(
                reinterpret_cast<const char *>(rawData) + (size * streamCnt),
                reinterpret_cast<const char *>(rawData) + size +
                    (size * streamCnt));

            if (pFsfWriteRaw->SetStream(frameCtr, streamCnt, streamRaw) !=
                aditof::FsfStatus::SUCCESS) {
                LOG(ERROR) << "Could not set stream.";
                recordFSFThreadFail();
                break;
...
void ADIToFRecorder::playbackFSFThread() {
    ...
    while (!m_playbackThreadStop) {
        ...
        if ((frameDataLocationIR != nullptr &&
             frameDataLocationDEPTH != nullptr) &&
            (_prevWidth == streamInfo.nColsPerStream &&
             _prevHeight == streamInfo.nRowsPerStream)) {
            ...
            if ((currentPBPos < (fileHeader.nFrames - 1)) && !isPaused) {
                ...
                if (_streamEnable.active_br) {
                    pFsfRead->GetStream(
                        ...
                        streamTable
                            .find((uint32_t)
                                      aditof::StreamType::STREAM_TYPE_ACTIVE_BR)
                            ->second,
                        stream);
                    memcpy(reinterpret_cast<char *>(frameDataLocationIR),
                           stream.streamData.data(), stream.streamData.size());
                    //stream.streamData.clear();
                }
                ...
                if (_streamEnable.radial) {
                    pFsfRead->GetStream(
                        ...
                        streamTable
                            ...
                            ->second,
                        stream);
                    memcpy(reinterpret_cast<char *>(frameDataLocationDEPTH),
                           stream.streamData.data(), stream.streamData.size());
                    //stream.streamData.clear();
                }
                if (_streamEnable.x &&
                    _streamEnable.y) //This must be a Point Cloud
                {
                    processXYZData();
                }
                currentPBPos++;
            } else //loop around, and keep last stream showing
            {
                ...
                if (_streamEnable.x &&
                    _streamEnable.y) //Get Point Cloud Information
                {
                    processXYZData();
                }
            }
...
void ADIToFRecorder::processXYZData() {
    size_t imageSize =
        m_frameDetails.width * m_frameDetails.height * sizeof(uint16_t);
    //local variables
    ...
    //Get X
    int xIndex =
        streamTable.find((uint32_t)aditof::StreamType::STREAM_TYPE_X)->second;
    pFsfRead->GetStream(currentPBPos, xIndex, stream);
    memcpy(reinterpret_cast<char *>(xData), stream.streamData.data(),
           stream.streamData.size());
    //stream.streamData.clear();

    //Get Y
    int yIndex =
        streamTable.find((uint32_t)aditof::StreamType::STREAM_TYPE_Y)->second;
    pFsfRead->GetStream(currentPBPos, yIndex, stream);
    memcpy(reinterpret_cast<char *>(yData), stream.streamData.data(),
           stream.streamData.size());
    //stream.streamData.clear();

    //Get Z
    int zIndex =
        streamTable.find((uint32_t)aditof::StreamType::STREAM_TYPE_DEPTH)
            ->second;
    pFsfRead->GetStream(currentPBPos, zIndex, stream);
    memcpy(reinterpret_cast<char *>(zData), stream.streamData.data(),
           stream.streamData.size());
    //stream.streamData.clear();

    //Now that we have X, Y, and Z streams, we need to put all data together to form XYZ stream
    size_t pcSize =
        m_frameDetails.width * m_frameDetails.height * sizeof(aditof::Point3I);
    size_t depthCnt = 0;
    size_t xCnt = 0;
    size_t yCnt = 0;
    size_t zCnt = 0;
