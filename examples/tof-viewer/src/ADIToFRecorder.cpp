...
ADIToFRecorder::ADIToFRecorder()
	: m_frameDetails{ 0, 0, "", {0.0f, 1.0f} }, m_recordTreadStop(true),
...
{		
	...
	while (!m_playbackThreadStop) 
	{
		...
		frame->getData(aditof::FrameDataType::IR, &frameDataLocationIR);
		frame->getData(aditof::FrameDataType::DEPTH, &frameDataLocationDEPTH);
...
void ADIToFRecorder::clearVariables()
{
	...
	m_frameDetails.cal_data = { 0, 1.0 };
	m_frameDetails.mode = 0;
