...
v4l2-ctl --set-ctrl=confidence_bits=1 -d /dev/v4l-subdev1
v4l2-ctl --set-ctrl=ab_averaging=1 -d /dev/v4l-subdev1
v4l2-ctl --set-ctrl=depth_enable=1 -d /dev/v4l-subdev1
v4l2-ctl --device /dev/video0 --set-fmt-video=width=2048,height=512,pixelformat=BA81 --stream-mmap --stream-to=frame.bin --stream-count=$nr_frames
