ADI provide 2 executables (built for ARM64) to interract with imager Flash memory through ADSD3500
These 2 tools can be used in both host-boot usecase and self-boot.
For host-boot please check host_boot.sh

	1. FW_UPDATE - is used to flash a new image into the imager SPI flash memory.
...
If module is not self-booting because Flash memory content is corrupted or does not contain ADSD3500 firmware, FW_UPDATE tool can be used to fix it.
